...
module.exports = {
  ...
  laporanRekapPembayaran: promiseHandler(async (req, res, next) => {
    ...
    return helpers.response(res, 200, "Get Rekap Pembayaran siswa berhasil", resultWithStudent);
  }),
  laporanKasBank: promiseHandler(async (req, res, next) => {
    ...
    const resultKreditPrev = await creditModel.getAllKreditSubmittedWithDate(true, '1-10102', tanggal_awal, tanggal_akhir, unit_id);
    ...
    const reducedArrPrev = [...resultKreditPrev, ...resultDebitPrev].reduce((acc, cur) => {
      acc[cur.account_cost_account_code] ? acc[cur.account_cost_account_code].total = parseInt(acc[cur.account_cost_account_code].total, 10) + parseInt(cur.total, 10) : acc[cur.account_cost_account_code] = cur;
      acc[cur.account_cost_account_code] ? acc[cur.account_cost_account_code].total_keluar = (parseInt(acc[cur.account_cost_account_code].total_keluar, 10) + parseInt(cur.total_keluar, 10)) || 0 : acc[cur.account_cost_account_code] = cur;
      return acc;
    }, {});
    ...
    const resultKredit = await creditModel.getAllKreditSubmittedWithDate(false, '1-10102', tanggal_awal, tanggal_akhir, unit_id);
    ...
    const newResult = { data_payment: combinedResult, sub_total_masuk: subTotalMasuk, sub_total_keluar: subTotalKeluar, ...newResultCashAccount, total_masuk: subTotalMasuk + newResultCashAccount.saldo_awal_debit, total_keluar: subTotalKeluar + newResultCashAccount.saldo_awal_kredit, saldo_akhir: (subTotalMasuk + newResultCashAccount.saldo_awal_debit) - (subTotalKeluar + newResultCashAccount.saldo_awal_kredit) }
    return helpers.response(res, 200, "Get Laporan kas bank berhasil", newResult);
  ...
  laporanKasTunai: promiseHandler(async (req, res, next) => {
    ...
    const resultKreditPrev = await creditModel.getAllKreditSubmittedWithDate(true, '1-10101', tanggal_awal, tanggal_akhir, unit_id);
    ...
    const resultKredit = await creditModel.getAllKreditSubmittedWithDate(false, '1-10101', tanggal_awal, tanggal_akhir, unit_id);
    ...
    return helpers.response(res, 200, "Get Laporan kas tunai berhasil", newResult);
